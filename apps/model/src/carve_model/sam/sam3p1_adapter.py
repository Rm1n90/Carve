...
# Phase 5 — Task 5.1: deleted the ``_NATIVE_IMAGE_PREDICTOR`` module-level
# singleton, ``_get_or_build_native_image_predictor``,
# ``_set_native_image_predictor_for_tests``, and ``reset_native_image_predictor``.
# That second sam3.1 adapter copy was the source of the v3 OOM bug. The
# lifecycle manager (``carve_model.sam.lifecycle.Sam3p1Variant``) now owns
# the sole adapter via ``self._adapter`` and handles eviction through
# ``manager.force_unload()`` / ``manager.evict_if_idle()``.
