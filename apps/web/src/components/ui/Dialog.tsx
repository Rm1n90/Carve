 ...
 * native Radix open/close logic handles teardown (no `forceMount`). Animation
 * is enter-only via framer-motion's `initial`/`animate`. The exit animation
 * was dropped intentionally — see /tmp/v21-audit.md bug A: combining
 * `forceMount` with `AnimatePresence` left closed dialogs visible because
 * there was no CSS rule for `[data-state="closed"]`.
 */
